...
public class TEWritingTable extends TileEntity implements IInventory
{
	...
	public void updateEntity()
	{
		...
		if (containsPotionFlask() && containsRegisteredPotionIngredient())
		{
			if (containsCatalyst())
			{
				if (getStackInSlot(6) == null)
				{
					...
					{
						ItemStack flaskStack = inv[getPotionFlaskPosition()];
						ItemStack ingredientStack = inv[getRegisteredPotionIngredientPosition()];
						ItemStack catalystStack = inv[getCatalystPosition()];
						...
						setInventorySlotContents(6, flaskStack);
						decrStackSize(getPotionFlaskPosition(), 1);
						decrStackSize(getCatalystPosition(), 1);
						decrStackSize(getRegisteredPotionIngredientPosition(), 1);
						progress = 0;
			...
			else if (containsBindingAgent())
			{
				if (getStackInSlot(6) == null)
				{
					...
					if (progress >= progressNeeded)
					{
						ItemStack flaskStack = inv[getPotionFlaskPosition()];
						ItemStack ingredientStack = inv[getRegisteredPotionIngredientPosition()];
						ItemStack agentStack = inv[getBindingAgentPosition()];
						...
						decrStackSize(getPotionFlaskPosition(), 1);
						decrStackSize(getBindingAgentPosition(), 1);
						decrStackSize(getRegisteredPotionIngredientPosition(), 1);
						progress = 0;
		...
		else if (containsBlankSlate() && containsPotionFlask())
		{
			if (getStackInSlot(6) == null)
			{
				progress++;
				...
				if (progress >= progressNeeded)
				{
					...
					ItemStack blankSlate = inv[getBlankSlatePosition()];
					...
					setInventorySlotContents(6, flaskStack);
					decrStackSize(getPotionFlaskPosition(), 1);
					decrStackSize(getBlankSlatePosition(), 1);
		...
		else if (containsFillingAgent() && containsPotionFlask())
		{
			//TODO
			if (getStackInSlot(6) == null)
			{
				...
				if (progress >= progressNeeded)
				{
					ItemStack flaskStack = inv[getPotionFlaskPosition()];
					//ItemStack ingredientStack = inv[this.getRegisteredPotionIngredientPosition()];
					ItemStack fillingAgent = inv[getFillingAgentPosition()];
					...
					setInventorySlotContents(6, flaskStack);
					decrStackSize(getPotionFlaskPosition(), 1);
					decrStackSize(getFillingAgentPosition(), 1);
					//this.decrStackSize(this.getRegisteredPotionIngredientPosition(), 1);
					progress = 0;
		...
		else
		{
			...
			{
				progress = 0;
				amountUsed = getAmountNeeded(getStackInSlot(0));
			...
			if (getStackInSlot(6) == null)
			{
				...
				{
					progress = 0;
					setInventorySlotContents(6, getResultingItemStack());
