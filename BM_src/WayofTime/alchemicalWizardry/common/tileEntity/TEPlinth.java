...
public class TEPlinth extends TileEntity implements IInventory
{
	...
	{
		inv = new ItemStack[1];
		ring1Inv = new ItemStack[6];
		ring2Inv = new ItemStack[6];
		ring3Inv = new ItemStack[6];
		isActive = false;
		progress = 0;
		progressInterval = 50;
	}
		...
		else
		{
			if (getStackInSlot(0) != null && getStackInSlot(0).getItem() instanceof EnergyBattery)
			{
				if (progress % progressInterval == 0)
				{
					...
					{
					case 1:
						itemStack = ring1Inv[slot];
						break;

					case 2:
						itemStack = ring2Inv[slot];
						break;

					case 3:
						itemStack = ring3Inv[slot];
						break;
					...
					{
						if (deleteItemStackInRing(ring, itemStack))
						{
							progress++;
						}
					}
