...
//        setDropOfActivatedSword(stack, level >= 0 ? soulDrop[level] : 0);
    ...
    public EntityTippedArrow getArrowEntity(World world, ItemStack stack, EntityLivingBase target, EntityLivingBase user, float velocity) {
        ...
        EntitySentientArrow entityArrow = new EntitySentientArrow(world, user, type, amount, getLevel(soulsRemaining), null);
                ...
                if ((double) arrowVelocity >= 0.1D) {
                    ...
                    if (!world.isRemote) {
                        ...
                        if (itemarrow == Items.ARROW) {
                            double soulsRemaining = PlayerDemonWillHandler.getTotalDemonWill(type, player);
                            entityArrow = new EntitySentientArrow(world, entityLiving, type, amount, getLevel(soulsRemaining), null);
