 ...
 * @author WayofTime
 */
public class BasicCompositeFilter implements IItemFilter
{
	/*
	...
	public void initializeFilter(List<IFilterKey> filteredList, BlockEntity tile, IItemHandler itemHandler, boolean isFilterOutput)
	{
        ...
        int maxCount = 0;
        for (IFilterKey key : requestList) {
            maxCount = Math.max(maxCount, key.getCount());
        }
        this.maxCount = maxCount;

        for (int i = 0; i < itemHandler.getSlots(); i++) {
            ItemStack testStack = itemHandler.getStackInSlot(i);
            if (testStack.isEmpty()) {
                continue;
            }
            if (doesStackPassFilter(testStack)) {
                maxCount -= testStack.getCount();
            }
        }
	...
	{
		int allowedAmount = 0;
        if (doesStackPassFilter(inputStack)) {
            allowedAmount = Math.min(maxCount, inputStack.getCount());
        }
	...
	public int transferThroughInputFilter(IItemFilter outputFilter, int maxTransfer)
	{
		...
		for (int slot = 0; slot < itemHandler.getSlots(); slot++)
		{
            ...
            if (doesStackPassFilter(inputStack))
            {
                allowedAmount = Math.min(maxTransfer, Math.min(maxCount, itemHandler.extractItem(slot, inputStack.getCount(), true).getCount()));
            }
