...
//		BloodAltarRecipeBuilder.altar(Ingredient.fromItems(Items.BUCKET), new ItemStack(BloodMagicBlocks.LIFE_ESSENCE_BUCKET.get()), AltarTier.ONE.ordinal(), 1000, 5, 0).build(consumer, BloodMagic.rl(basePath + "bucket_life"));
		...
		LivingDowngradeRecipeBuilder.downgrade(Ingredient.fromItems(Items.SOUL_SAND), LivingArmorRegistrar.DOWNGRADE_SPEED_DECREASE.get().getKey()).build(consumer, BloodMagic.rl(basePath + "speed_decrease"));
		LivingDowngradeRecipeBuilder.downgrade(Ingredient.fromItems(Items.SHIELD), LivingArmorRegistrar.DOWNGRADE_CRIPPLED_ARM.get().getKey()).build(consumer, BloodMagic.rl(basePath + "crippled_arm"));
	}
}
